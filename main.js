...
// -------------------------------------------------------------
// REPLACEMENT FOR FETCH: Hardcoded data object
// -------------------------------------------------------------
const customizeData = {
  "name": "xyz",
...
const applyData = () => {
  Object.keys(customizeData).forEach((key) => {
    if (customizeData[key] !== "") {
      if (key === "imagePath") {
        const img = document.getElementById(key);
        if(img) img.setAttribute("src", customizeData[key]);
      } else {
        const txt = document.getElementById(key);
        if(txt) txt.innerText = customizeData[key];
      }
    }
  });
};
